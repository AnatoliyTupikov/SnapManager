...
(function (root, factory) {
    ...
    // Ссылка на window и document (если они есть)
    var _window = typeof window !== "undefined" ? window : null;
    var _isServer = _window === null;
    var _document = _isServer ? undefined : _window.document;

    // Функция-заглушка
    var noop = function () { return false; };

    // CSS calc с префиксами
    var calc = _isServer ? "calc" : ["", "-webkit-", "-moz-", "-o-"].filter(function (prefix) {
    ...
    var getElement = function (el) {
        if (isString(el)) {
            var found = _document.querySelector(el);
            if (!found) throw new Error("Selector " + el + " did not match a DOM element");
            return found;
        }
        return el;
    };

    var defaultValue = function (obj, prop, fallback) {
        return obj[prop] !== undefined ? obj[prop] : fallback;
    };

    // Создание разделителя (gutter)
    var createGutter = function (index, direction) {
        var div = _document.createElement("div");
        div.className = "gutter gutter-" + direction;
        return div;
    };

    // Применение стилей к элементу
    var setElementStyle = function (el, prop, size, gutterSize) {
        var styleObj = {};
        if (isString(size)) {
            styleObj[prop] = size;
        } else {
            styleObj[prop] = calc + "(" + size + "% - " + gutterSize + "px)";
        }
    ...
    // Основная функция Split
    return function (elements, options) {
        if (options === undefined) options = {};

        if (_isServer) return {}; // на сервере ничего не делаем

        var elList = Array.from ? Array.from(elements) : elements;
        var parent = getElement(elList[0]).parentNode;

        // Получаем опции
        var sizes = defaultValue(options, "sizes", elList.map(() => 100 / elList.length));
        var minSize = defaultValue(options, "minSize", 100);
        var gutterSize = defaultValue(options, "gutterSize", 10);
        var gutterAlign = defaultValue(options, "gutterAlign", "center");
        var direction = defaultValue(options, "direction", "horizontal");

        // Массив для хранения информации о каждом элементе
        var elementsData = elList.map(function (el, index) {
            return {
                element: getElement(el),
                size: sizes[index],
                minSize: Array.isArray(minSize) ? minSize[index] : minSize,
                i: index
            };
        });

        // Добавление разделителей (gutter) и обработчиков
        var gutters = [];
        for (var i = 1; i < elementsData.length; i++) {
            var gutter = createGutter(i, direction);
            parent.insertBefore(gutter, elementsData[i].element);
            gutters.push(gutter);
            // здесь добавляются обработчики drag (mousedown, mousemove, mouseup)
        }
        ...
        return {
            ...
            getSizes: function () {
                return elementsData.map(function (el) { return el.size; });
            },
            collapse: function (index) {
                // коллапс элемента
            },
            destroy: function (removeGutters, resetSizes) {
                // удаление разделителей и очистка стилей
            },
            parent: parent,
            pairs: gutters
        };
